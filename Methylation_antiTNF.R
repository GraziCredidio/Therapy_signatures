...
# Anti-TNF x No Biologics - remitters + crp significant genes
...
transcriptome_data <- read.table("Output_files/DESeq2/Maaslin2/Remission_crp/DESeq2_normalized_antiTNF_R_crp_04.05.txt", sep = "\t")
transcriptome_info_file <- read.table("Output_files/Maaslin2/Tables/Remission_crp/coldata_maaslin2_antiTNF_R_crp_04.05.txt", sep = "\t") #coldata
...
deg_gene_list <- read.table("Output_files/Maaslin2/Results/Remission_crp/significant_results/maaslin2_significant_results_antiTNF_vs_noBio_R_crp.txt",
                            sep = '\t')
...
write.table(gene_meth_site_correlation, file = "Output_files/Methylation/antiTNF/antiTNF_DEG_meth_site_correlation_5000bp_1000rep.txt", quote = FALSE,
            sep = '\t')


#Extract significant DEGs  ----
#gene_meth_site_correlation <- read.table("Output_files/Methylation/antiTNF_DEG_meth_site_correlation_5000bp_1000rep.txt", sep = '\t')

sig_gene_meth_site_correlation <- gene_meth_site_correlation %>% #118
  filter(FDR < 0.05)
...
write.table(sig_gene_meth_site_correlation, file = "Output_files/Methylation/antiTNF/antiTNF_significant_DEG_meth_site_correlation_5000bp_1000rep.txt", quote = FALSE,
            sep = '\t')


# split into up and downregulated significant genes from correlation
sig_genes <- read.table("Output_files/Maaslin2/Results/Remission_crp/significant_results/maaslin2_significant_results_antiTNF_vs_noBio_R_crp.txt",
                        sep = '\t')
...
write.table(up_sig_gene_meth_site_correlation, file = "Output_files/Methylation/antiTNF/antiTNF_up_significant_DEG_meth_site_correlation_5000bp_1000rep.txt", quote = FALSE,
            sep = '\t')
...
write.table(down_sig_gene_meth_site_correlation, file = "Output_files/Methylation/antiTNF/antiTNF_down_significant_DEG_meth_site_correlation_5000bp_1000rep.txt", quote = FALSE,
            sep = '\t')


#GO term enrichment analysis --------
#loading files
sig_gene_meth_site_correlation <- read.table("Output_files/Methylation/antiTNF/antiTNF_significant_DEG_meth_site_correlation_5000bp_1000rep.txt",
                                             sep = '\t')
up_sig_gene_meth_site_correlation <- read.table("Output_files/Methylation/antiTNF/antiTNF_up_significant_DEG_meth_site_correlation_5000bp_1000rep.txt",
                                                sep = '\t')
down_sig_gene_meth_site_correlation <- read.table("Output_files/Methylation/antiTNF/antiTNF_down_significant_DEG_meth_site_correlation_5000bp_1000rep.txt", 
                                                  sep = '\t')
transcriptome_data <- read.table("Output_files/DESeq2/Maaslin2/Remission_crp/DESeq2_normalized_antiTNF_R_crp_04.05.txt", sep = "\t")
...
for (i in 1:length(direction)) {
  ...
  write.table(go_results, paste("Output_files/Methylation/antiTNF/GO/antiTNF_correlated_", direction, "_DEGs_GO.txt", sep = ''), 
              sep = '\t', quote = FALSE)
  
}


up_go_result <- read.csv("Output_files/Methylation/antiTNF/GO/antiTNF_correlated_up_DEGs_GO.txt", sep = '\t')
down_go_result <- read.csv("Output_files/Methylation/antiTNF/GO/antiTNF_correlated_down_DEGs_GO.txt", sep = '\t')
...
pdf("Output_files/Methylation/antiTNF/GO/dotplot_GO_BP_antiTNF_R_crp_data_integration.pdf", width = 9, height = 6)
dot_plot <- ggplot(selected_GO, aes(x = direction, y= Term, size = geneRatio, color = Fisher.elim)) +
  ...
  xlab('') + ylab('') +
  labs(
    title = "GO BP AntiTNF x No Biologics",
