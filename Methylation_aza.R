...
# Aza x No syst - remitters + crp significant genes
...
transcriptome_data <- read.table("Output_files/DESeq2/Maaslin2/Remission_crp/DESeq2_normalized_aza_R_crp_04.05.txt", sep = "\t")
transcriptome_info_file <- read.table("Output_files/Maaslin2/Tables/Remission_crp/coldata_maaslin2_aza_R_crp_04.05.txt", sep = "\t") #coldata
...
deg_gene_list <- read.table("Output_files/Maaslin2/Results/Remission_crp/significant_results/maaslin2_significant_results_aza_vs_noSyst_R_crp.txt",
                            sep = '\t')
...
write.table(gene_meth_site_correlation, file = "Output_files/Methylation/aza/aza_DEG_meth_site_correlation_5000bp_1000rep.txt", quote = FALSE,
            sep = '\t')


#Extract significant DEGs  ----
#gene_meth_site_correlation <- read.table("Output_files/Methylation/DEG_meth_site_correlation_5000bp_1000rep.txt", sep = '\t')

sig_gene_meth_site_correlation <- gene_meth_site_correlation %>% #1553
  filter(FDR < 0.05)
...
write.table(sig_gene_meth_site_correlation, file = "Output_files/Methylation/aza/aza_significant_DEG_meth_site_correlation_5000bp_1000rep.txt", quote = FALSE,
            sep = '\t')


# split into up and downregulated significant genes from correlation
sig_genes <- read.table("Output_files/Maaslin2/Results/Remission_crp/significant_results/maaslin2_significant_results_aza_vs_noSyst_R_crp.txt",
                        sep = '\t')
...
write.table(up_sig_gene_meth_site_correlation, file = "Output_files/Methylation/aza/aza_up_significant_DEG_meth_site_correlation_5000bp_1000rep.txt", quote = FALSE,
            sep = '\t')
...
write.table(down_sig_gene_meth_site_correlation, file = "Output_files/Methylation/aza/aza_down_significant_DEG_meth_site_correlation_5000bp_1000rep.txt", quote = FALSE,
            sep = '\t')


#GO term enrichment analysis: correlated genes --------
#loading files
sig_gene_meth_site_correlation <- read.table("Output_files/Methylation/aza/aza_significant_DEG_meth_site_correlation_5000bp_1000rep.txt",
                                             sep = '\t')
up_sig_gene_meth_site_correlation <- read.table("Output_files/Methylation/aza/aza_up_significant_DEG_meth_site_correlation_5000bp_1000rep.txt",
                                                sep = '\t')
down_sig_gene_meth_site_correlation <- read.table("Output_files/Methylation/aza/aza_down_significant_DEG_meth_site_correlation_5000bp_1000rep.txt", 
                                                  sep = '\t')
transcriptome_data <- read.table("Output_files/DESeq2/Maaslin2/Remission_crp/DESeq2_normalized_aza_R_crp_04.05.txt", 
                                 sep = "\t")
...
for (i in 1:length(direction)) {
  ...
  write.table(go_results, paste("Output_files/Methylation/aza/GO/aza_correlated_", direction, "_DEGs_GO.txt", sep = ''), 
              sep = '\t', quote = FALSE)
  
}


up_go_result <- read.table("Output_files/Methylation/aza/GO/aza_correlated_up_DEGs_GO.txt", sep = '\t')
down_go_result <- read.table("Output_files/Methylation/aza/GO/aza_correlated_down_DEGs_GO.txt", sep = '\t')

#Plot selected GO terms enriched in DEGs 
terms <- c("erythrocyte maturation", "negative regulation of potassium ion transmembrane transport", "stem cell proliferation", 
           "female pregnancy", "amino acid import across plasma membrane",
           
           "positive regulation of natural killer cell mediated cytotoxicity", "chemokine-mediated signaling pathway",
           "monocyte chemotaxis", "inflammatory response", "T-helper 17 cell lineage commitment", "tumor necrosis factor production",
           "cell population proliferation", "negative regulation of I-kappaB kinase/NF-kappaB signaling", "negative regulation of T cell cytokine production")
...
pdf("Output_files/Methylation/aza/GO/dotplot_GO_BP_aza_R_crp_data_integration.pdf", width = 9, height = 6)
...
for (i in 1:length(direction)) {
  ...
  write.table(go_results, paste("Output_files/Methylation/aza/GO/correlated_topVar/aza_correlated_topVar_", direction, "_DEGs_GO.txt", sep = ''), 
              sep = '\t', quote = FALSE)
  
}


up_go_result <- read.table("Output_files/Methylation/aza/GO/correlated_topVar/aza_correlated_topVar_up_DEGs_GO.txt", sep = '\t')
down_go_result <- read.table("Output_files/Methylation/aza/GO/correlated_topVar/aza_correlated_topVar_down_DEGs_GO.txt", sep = '\t')
names(up_go_result)
...
<<<<<<< HEAD
pdf("Output_files/Methylation/aza/GO/correlated_topVar/dotplot_GO_BP_aza_R_crp_data_integration_topVar.pdf", width = 10, height = 8)
=======
...
pdf("Output_files/Methylation/aza/GO/correlated_topVar/dotplot_GO_BP_aza_R_crp_data_integration_topVar2.pdf", width = 10, height = 8)
>>>>>>> ff6458e797c8a20bc7fdc4e81692e6ad81e27aec
dot_plot <- ggplot(selected_GO, aes(x = direction, y= Term, size = geneRatio, color = Fisher.elim)) +
  geom_point() +
  scale_colour_gradient(low = "#990000", high = "#FF9999") +
  #scale_size_continuous(range=c(3,10)) +
