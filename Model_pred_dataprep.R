...
write.table(ord.coldata_all, "Cleaned_tables/models/pred/EZECohort_coldata_pred_allPatients.txt", sep = "\t", row.names = TRUE)
...
write.table(ord.coldata_inactive, "Cleaned_tables/models/pred/EZECohort_coldata_pred.txt", sep = "\t", row.names = TRUE)

# Normalizing counts ----
deseq_norm <- function(counts, coldata){
  dds_counts <- DESeqDataSetFromMatrix(countData = counts,
                                       colData = coldata,
                                       design = ~ diagnosis_class + sex + bmi_class + age_group + pred_vs_noSyst)
...
write.table(df_normalized_counts, "Cleaned_tables/models/pred/normalized_counts_pred_allPatients.txt", sep = "\t",  quote = FALSE)

vst_counts <- vst(dds_counts, blind=TRUE) 
vst_counts <- assay(vst_counts)
write.table(vst_counts, "Cleaned_tables/models/pred/vst_counts_pred_allPatients.txt", sep = "\t",  quote = FALSE)
...
write.table(df_normalized_counts_inactive, "Cleaned_tables/models/pred/normalized_counts_pred.txt", sep = "\t",  quote = FALSE)

vst_counts_inactive <- vst(dds_counts_inactive, blind=TRUE) 
vst_counts_inactive <- assay(vst_counts_inactive)
write.table(vst_counts_inactive, "Cleaned_tables/models/pred/vst_counts_pred.txt", sep = "\t",  quote = FALSE)